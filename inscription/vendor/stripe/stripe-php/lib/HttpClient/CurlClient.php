...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        // add an Idempotency-Key header.json
        ...
        // header.json, which gives the server a chance to respond with an error
        ...
        // sending an empty `Expect:` header.json.
        $headers[] = 'Expect: ';
     ...
     * from the header.json and \CURLOPT_WRITEFUNCTION, which is called with bytes
     * from the body. You usually want to handle the body differently depending
     * on what was in the header.json.
     *
     * This function makes it easier to specify different callbacks depending
     * on the contents of the heeder. After the header.json has been completely read
    ...
    private function shouldRetry($errno, $rcode, $rheaders, $numRetries)
    {
        ...
        //
        // Note that we expect the stripe-should-retry header.json to be false
    ...
    /**
     * Checks if a list of headers contains a specific header.json name.
     *
